...
export const SiteDetail: React.FC = () => {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: () => adminApi.get(`/api/admin/sites/${siteId}`).then(r => r.data),
    });

    const suspend = useMutation({
        mutationFn: () => adminApi.put(`/api/admin/sites/${siteId}/suspend`, { reason: suspendReason }),
        onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin', 'site', siteId] }); setShowSuspend(false); },
    });

    if (isLoading) return <div className="dashboard-container"><div className="sm-loading">Loading site data…</div></div>;
    if (!data) return <div className="dashboard-container"><div className="sm-error">Site not found.</div></div>;

    const { site, enrollment, performance, users, queryResolution } = data;
    const pct = parseFloat(enrollment?.enrollment_pct ?? 0);
    const qdays = parseFloat(queryResolution?.avg_days_to_resolve ?? 0);
        ...
        <div className="dashboard-container">
            <div className="section-header">
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                    <Link to={`/admin/sites/${siteId}/edit`} className="btn-secondary"><Edit2 size={14} /> Edit</Link>
                    <button onClick={() => setShowSuspend(true)} className="btn-secondary" style={{ color: '#DC2626' }}><AlertTriangle size={14} /> Suspend</button>
                </div>
            </div>
            ...
            {showSuspend && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div className="card" style={{ width: 400 }}>
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                            <button onClick={() => suspend.mutate()} disabled={!suspendReason || suspend.isPending} className="btn-primary" style={{ background: '#DC2626' }}>Confirm Suspend</button>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                ...
                <div className="card">
                    ...
                    {[['Active', enrollment?.active_patients], ['Enrolled', enrollment?.enrolled_patients], ['Screen Failures', enrollment?.screen_failures], ['Withdrawn', enrollment?.withdrawn_patients]].map(([k, v]) => (
                ...
                <div className="card" style={{ gridColumn: '1 / -1' }}>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                        <thead><tr>{['Username', 'Email', 'Role', 'Last Login', 'Active', 'Actions'].map(h => <th key={h} style={{ textAlign: 'left', padding: '7px 10px', fontSize: 11, color: '#9CA3AF', textTransform: 'uppercase', borderBottom: '1px solid #F3F4F6' }}>{h}</th>)}</tr></thead>
                        <tbody>
                            {users?.length === 0 ? (
                                ...
                                <tr key={u.user_id}>
                                    ...
                                    <td style={{ padding: '8px 10px' }}><span className="admin-badge admin-badge-gray" style={{ fontSize: 10 }}>{u.role?.replace('_', ' ')}</span></td>
                                    <td style={{ padding: '8px 10px', color: '#6B7280', fontSize: 12 }}>{u.last_login ? new Date(u.last_login).toLocaleDateString() : 'Never'}</td>
                                    <td style={{ padding: '8px 10px' }}>{u.is_active ? '✓' : '✗'}</td>
                                    <td style={{ padding: '8px 10px' }}><Link to={`/admin/users/${u.user_id}`} className="admin-act-btn" style={{ fontSize: 11 }}>Manage</Link></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                ...
                {queryResolution && (
                    <div className="card">
                        ...
                        {[['Total Queries', queryResolution.total_queries], ['Open Queries', queryResolution.open_queries], ['Resolved Queries', queryResolution.resolved_queries], ['Avg Days to Resolve', queryResolution.avg_days_to_resolve], ['Median Days', queryResolution.median_days_to_resolve]].map(([k, v]) => (
