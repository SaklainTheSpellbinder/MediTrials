            ...
            await Promise.all([
                ...
                pool.query(`
                    SELECT ad.dataset_id, ad.dataset_name, ad.dataset_type, ad.snapshot_date,
                           ...
                           ct.trial_title
                    FROM meditrials.analysis_datasets ad
                    JOIN meditrials.clinical_trials ct ON ct.trial_id = ad.trial_id
                ...
                pool.query(`
                    ...
                    FROM meditrials.survival_analysis sa
                    JOIN meditrials.clinical_trials ct ON ct.trial_id = sa.trial_id
                ...
                pool.query(`
                    ...
                    WHERE p.patient_status IN ('Active','Completed')
                      AND NOT EXISTS (
                          SELECT 1 FROM meditrials.protocol_deviations pd
                ...
                pool.query(`
                    SELECT trial_id FROM meditrials.clinical_trials
                ...
                pool.query(`
                    ...
                    FROM meditrials.mv_randomization_balance
        ...
        if (firstTrial.rows.length > 0) {
            try {
                const { rows: pr } = await pool.query(`
                    ...
                    FROM (CALL meditrials.sp_calculate_power_analysis($1, 0.5, 0.05, 0.8, NULL, NULL)) AS r
...
router.post('/datasets/generate', async (req: Request, res: Response) => {
    ...
    try {
        const { rows } = await pool.query(`
            SELECT csdr_report FROM (CALL meditrials.sp_generate_csdr($1, NULL)) AS r
        `, [trial_id]);
...
router.get('/survival', async (req: Request, res: Response) => {
    ...
    try {
        const { rows } = await pool.query(`
            SELECT sa.*, ct.trial_title FROM meditrials.survival_analysis sa
            JOIN meditrials.clinical_trials ct ON ct.trial_id = sa.trial_id
...
router.post('/survival/calculate', async (req: Request, res: Response) => {
    ...
    try {
        await pool.query(`CALL meditrials.sp_calculate_survival($1, $2)`, [trial_id, endpoint_type]);
        const { rows } = await pool.query(`
            SELECT * FROM meditrials.survival_analysis
...
router.post('/power', async (req: Request, res: Response) => {
    ...
    try {
        const { rows } = await pool.query(`
            SELECT required_sample_size, current_power
            FROM (CALL meditrials.sp_calculate_power_analysis($1, $2, $3, $4, NULL, NULL)) AS r
...
router.get('/balance', async (req: Request, res: Response) => {
    ...
    try {
        const q = trial_id
            ? await pool.query(`SELECT * FROM meditrials.mv_randomization_balance WHERE trial_id = $1 ORDER BY arm_code`, [trial_id])
            : await pool.query(`SELECT * FROM meditrials.mv_randomization_balance ORDER BY trial_id, arm_code`);
        res.json(q.rows);
    } catch (err: any) {
        res.status(500).json({ error: err.message });
    }
...
router.get('/ae-incidence', async (req: Request, res: Response) => {
    ...
    try {
        const { rows } = await pool.query(
            `SELECT * FROM meditrials.mv_ae_by_arm WHERE trial_id = $1 ORDER BY arm_code`,
            [trial_id]
        );
        res.json(rows);
    } catch (err: any) {
